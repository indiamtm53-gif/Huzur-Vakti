...
export default function App() {
  ...
  const [aktifTab, setAktifTab] = useState('vakitler'); // 'vakitler' veya 'zikirmatik'

  // Canlı Ezan Vakitlerini Çekme
  useEffect(() => {
  ...
  const vakitleriGetir = async (sehir) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // API yedek varsayılan veri (Bağlantı kesilirse)
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* Sekme Değiştirici (Vakitler / Zikirmatik) */}
      ...
      {aktifTab === 'vakitler' ? (
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.heroCard}>
            ...
            <Text style={styles.heroDetay}>Ezan vakti yaklaşınca bildirim alacaksınız.</Text>
          </View>
